var initDjangoHStoreWidget = function(hstore_field_name, inline_prefix) {
    ...
    if(hstore_field_name.indexOf('inline') > -1){
        var inlineClass = $('#id_'+hstore_field_name).parents('.inline-related, .form-group').attr('class');
    ...
    var compileUI = function(params){
        var hstore_field_id = 'id_'+hstore_field_name,
            ...
            original_container = original_textarea.parents('.form-group, .control-label, .form-row').eq(0),
        ...
        var hstore_field_data = {
                ...
                "help": original_container.find('.help-block, .grp-help, .help').text(),
                "errors": errorHtml,
                "data": json_data
            },
            // compile template
    ...
    var updateTextarea = function(container) {
        ...
        var new_value = {},
            ...
            rows = container.find('.form-row, .grp-row, .formset-row');
    ...
    $hstore.delegate('a.remove-row', 'click', function(e) {
        ...
        $(this).parents('.form-row, .grp-row, .formset-row').eq(0).remove();
